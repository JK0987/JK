...
function generateProfile() {
    ...
    if (maybeYourFriend) {
        ...
        if ($('#tipButton').get(0)) {
            ...
            const tipButton1 = document.createElement("button");
            tipButton1.id = "tipButton1";
            $(tipButton1).attr('title', 'Tip this hunter 10 SB+');
            $(tipButton1).text('Tip 10 SB+');
            yourFriendsProfile.prepend(tipButton1);
            $(tipButton1).css({
                'background-image': "url('https://www.toptal.com/designers/subtlepatterns/patterns/interlaced.png')",
                'background-repeat': 'no-repeat',
                'background-size': 'contain',
...
$(document).on('click', '#tipButton1', function() {
    const debug = localStorage.getItem('ws.debug');
    const receivingHunter = $('.userInteractionButtonsView-relationship').attr('data-recipient-snuid');
    const receivingName = $('.friendsPage-friendRow-titleBar-name').attr('data-text');
    const sendingHunter = user.unique_hash;
    const url = 'https://www.mousehuntgame.com/managers/ajax/users/supplytransfer.php?/sn=Hitgrab&hg_is_ajax=1&receiver='+receivingHunter+'&uh='+sendingHunter+'&item=super_brie_cheese&item_quantity=1';
    GM_xmlhttpRequest({
        method: "POST",
        url: url,
        onload: function(response) {
            ...
            alert('1 SB+ sent to '+receivingName);
        },
        onerror: function(response) {
            if (debug == true) {
                console.log('Tip No Good, Error',receivingName,receivingHunter,url);
